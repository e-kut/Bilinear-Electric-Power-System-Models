function [sys_init,sys_eq,x0,u0] = s3stg(varargin)
%----------------------------- Initialization -----------------------------
%                        Example 12.3 p.752 Kundur
...
%--------------------- Kundur's Linearization Method ----------------------
%                            (pp. 742-744)
n1 = E_B * (R_T * sin(delta_0r) + X_Td * cos(delta_0r)) / K_XR;
m1 = E_B * (X_Tq * sin(delta_0r) - R_T * cos(delta_0r)) / K_XR;
m2 = X_Tq * L_ads / (K_XR * (L_ads + L_fd));
n2 = R_T * L_ads / (K_XR * (L_ads + L_fd));
K1 = n1 * (psi_ad0 + L_aqs * i_d0) - m1*(psi_aq0 + L_ads_ht * i_q0);
K2 = n2 * (psi_ad0 + L_aqs * i_d0) - m2 * (psi_aq0 + L_ads_ht * i_q0) ...
    + L_ads_ht / L_fd * i_q0;
a11 = -K_D / (2*H);
a12 = -K1/(2*H);
a13 = -K2/(2*H);
a21 = omega_0;
a32 = - omega_0 * R_fd / L_fd * m1 * L_ads_ht;
a33 = - omega_0 * R_fd / L_fd * (1- L_ads_ht/L_fd + m2 * L_ads_ht);
Akund = [a11 a12 a13; a21 0 0; 0 a32 a33];
%------------------ End of Kundur's Linearization Method ------------------
