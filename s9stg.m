function [sys_init,sys_eq,x0,u0] = s9stg(varargin)
%----------------------------- Initialization -----------------------------
%    Example 12.3 p.752 + Example 12.4 p.775 + Example 12.5, p.789 Kundur
...
T_R = 0.02;     % AVR (terminal voltage tansducer) time constant [sec]
K_STAB = 9.5;   % PSS gain, [p.u]
T_W = 1.4;      % washout time constant [sec]
T_1 = 0.154;    % phase compensation time constant [sec]
T_2 = 0.033;    % phase compensation time constant [sec]
...
% Steady-state value of the PSS variable:
nu2_0 = 0;
nus_0 = 0;
% Operating point:
x0 = [0; delta_0r; psi_fd0; psi_1d0; psi_1q0; psi_2q0; nu1_0; nu2_0; nus_0]; % states
...
dyn_eq{3,1} = @(x) -omega_0 * R_fd * (i_fd(x) + K_A / L_ad * (x(7) - x(9)));
dyn_eq{4,1} = @(x) -omega_0 * R_1d * i_1d(x);
dyn_eq{5,1} = @(x) -omega_0 * R_1q * i_1q(x);
dyn_eq{6,1} = @(x) -omega_0 * R_2q * i_2q(x);
dyn_eq{7,1} = @(x) (Et(x) - x(7)) / T_R;
dyn_eq{8,1} = @(x,u) K_STAB * (dyn_eq{1,1}(x) + inp_eq {1,1}(u)) - ...
    x(8) / T_W;
dyn_eq{9,1} = @(x,u) (T_1 * dyn_eq{8,1}(x,u) + x(8) - x(9)) / T_2;
...
sys_eq{7,1} = @(x,u) dyn_eq{7,1}(x);
sys_eq{8,1} = @(x,u) dyn_eq{8,1}(x,u);
sys_eq{9,1} = @(x,u) dyn_eq{9,1}(x,u);
