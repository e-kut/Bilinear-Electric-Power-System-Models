function [sys_init,sys_eq,x0,u0] = s6stgPSS(varargin)
%----------------------------- Initialization -----------------------------
%               Example 12.3 p.752 + Example 12.4 p.775
...
nus_0 = 0;
% Operating point:
x0 = [0; delta_0r; psi_fd0; nu1_0; nu2_0; nus_0]; % states
...
dyn_eq{3,1} = @(x) -omega_0 * R_fd * (i_fd(x) + K_A / L_ad * (x(4) - x(6)));
dyn_eq{4,1} = @(x) (Et(x) - x(4)) / T_R;
dyn_eq{5,1} = @(x,u) K_STAB * (dyn_eq{1,1}(x) + inp_eq {1,1}(u)) - ...
    x(5) / T_W;
dyn_eq{6,1} = @(x,u) (T_1 * dyn_eq{5,1}(x,u) + x(5) - x(6)) / T_2;
...
sys_eq{5,1} = @(x,u) dyn_eq{5,1}(x,u);
sys_eq{6,1} = @(x,u) dyn_eq{6,1}(x,u);
